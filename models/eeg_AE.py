...
class eeg_encoder(nn.Module):
    def __init__(self, 
        ...
        if skip_mode == "conv":
            ...
            self.skips_agg = LinearLayer(int(sum([in_channels]+[(in_channels/2)//(stride**idx) for idx in range(len(channels))]))+1, 1)
...
class eeg_decoder(nn.Module):
    def __init__(self, 
        ...
        assert layer_mode in ['conv', 'linear'], f"Layer Mode Can be  Conv or Linaer Now : {layer_mode}"
        assert block_mode in ['res', 'conv'], f"Layer Mode Can be  Conv or Linaer Now : {block_mode}"
        assert up_mode in ["trans", "near", "conv"], f"Mode is must be 'trans' or 'near', 'conv'! Now:{up_mode}" 
        assert pos_mode in ['sinusoidal', 'trunc'], f"Layer Mode Can be  Conv or Linaer Now : {pos_mode}"
        assert skip_mode in  ['conv', 'down', None], f"Skip Mode Can be conv or donw Now : {skip_mode}"
        ...
        # Set Seqeucne
        seqeunces = [int((out_channels/2)//(stride**idx)) for idx in range(len(channels))]
        ...
        for idx, (dim_in, dim_out) in enumerate(channels):
            ...
            up_chan = dim_out if skip_mode == "conv" else dim_in
            
            if not is_last:
                 # Make upsample
                self.ups.append(upsample(dim_in, up_chan))
            else:
                # Make upsample
                self.ups.append(upsample(dim_in, up_chan, kernel_size=4, stride=2, padding=1))
...
class eeg_AutoEncoder(nn.Module):
    ...
    def __init__(self, 
        ...
        self.Encoder = eeg_encoder( in_seq = in_seq, in_channels = in_channels, out_seq = out_seq,
